/*
   Stage 4 of loader 422e30ed... -- the credential stealer, in memory.
   ...
   The stage encodes pointers and handles against a per-instance cookie held at
   `[esi+0x6d8]` in its context struct, seeded with a hardcoded constant:
   ...
   That scheme is compiled in, so it is present wherever the stage is mapped and
   decrypted, whether or not the stealer ever ran. It is also what ties this
   stage to stage 3: the seeding instruction is byte-identical in
   `stage3_alloc_at540M_dc038cc7.xor9` at 0x1605f and in
   `stage4_mapped_b454edc7.xor9` at 0x1447e -- same offset, same constant, same
   `push esi / call` shape after it. Same framework, same author.
   ...
   MEASURED RESULTS
     ...
     matches   2/2   decrypted stage artifacts (stage4_mapped, stage3_alloc540M)
...
rule RingForge_FormBook_Stage4_ContextCookie
{
    meta:
        author = "RingForge"
        description = "Stage 4 credential stealer of the 422e30ed chain, by its [esi+0x6d8] context cookie"
