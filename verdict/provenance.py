...
def _baked() -> Any:
    """Build-time identity, read only when this is actually a frozen build.

    A PyInstaller bundle has no git history *and* no distribution metadata, so
    both of the answers below would be `None` -- an unidentifiable verdict, out
    of the envelope built to make verdicts identifiable. `ringforge/_build_info`
    is generated by `ringforge.spec` and carries them across.

    **The `sys.frozen` guard is not belt and braces.** The spec writes that
    module into the source tree, where it is gitignored but stays behind after
    a local build. Without the guard it answers for source runs too, so a
    developer who had once built an exe would get a stale commit reported for
    working-tree code -- and `test_a_missing_git_checkout_is_not_an_error`
    caught exactly that. Unfrozen, git is the only authority and `None` is the
    honest answer.
    """
    if not getattr(sys, "frozen", False):
        return None
    try:
        from ringforge import _build_info
...
def analyzer_version() -> str | None:
    ...
    version = _package_version("ringforge-workbench")
    if version:
        return version
    baked = _baked()
    return getattr(baked, "VERSION", None) if baked else None
...
def git_commit(root: str | Path | None = None) -> str | None:
    ...
    A frozen build has none either, which is why it is stamped at build time
    and read back here -- see `_baked`.
    """
    ...
    try:
        ...
        pass
    baked = _baked()
    return getattr(baked, "COMMIT", None) if baked else None
