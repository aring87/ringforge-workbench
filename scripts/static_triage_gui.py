...
class StaticTriageGUI(tk.Tk):
    def __init__(self) -> None:
        ...
        for v in [
            self.enable_extract, self.enable_subfiles, self.subfile_limit,
            self.skip_strings, self.strings_lite, self.recursive_rounds,
        ]:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Checkbutton(af, text="Enable subfiles triage", variable=self.enable_subfiles).grid(row=0, column=1, sticky="w", padx=(12, 0))
        ttk.Label(af, text="Subfile limit:").grid(row=1, column=0, sticky="w", pady=(8, 0))
        ttk.Spinbox(af, from_=0, to=200, textvariable=self.subfile_limit, width=8).grid(row=1, column=1, sticky="w", padx=(12, 0), pady=(8, 0))
        ttk.Checkbutton(af, text="Skip strings", variable=self.skip_strings).grid(row=2, column=0, sticky="w", pady=(8, 0))
        ttk.Checkbutton(af, text="Strings lite", variable=self.strings_lite).grid(row=2, column=1, sticky="w", padx=(12, 0), pady=(8, 0))
        ttk.Label(af, text="Recursive rounds:").grid(row=3, column=0, sticky="w", pady=(8, 0))
        ttk.Spinbox(af, from_=0, to=10, textvariable=self.recursive_rounds, width=8).grid(row=3, column=1, sticky="w", padx=(12, 0), pady=(8, 0))
        ttk.Label(af, textvariable=self.advanced_warning, foreground="#b45309").grid(row=4, column=0, columnspan=2, sticky="w", pady=(10, 0))
    ...
    def _apply_preset(self) -> None:
        ...
        if p == "Fast Triage":
            self.enable_extract.set(True); self.enable_subfiles.set(True); self.subfile_limit.set(5)
            self.skip_strings.set(False); self.strings_lite.set(True); self.recursive_rounds.set(3)
        elif p == "Deep Triage":
            self.enable_extract.set(True); self.enable_subfiles.set(True); self.subfile_limit.set(25)
            self.skip_strings.set(False); self.strings_lite.set(False); self.recursive_rounds.set(3)
        elif p == "Hash Only":
            self.enable_extract.set(False); self.enable_subfiles.set(False); self.subfile_limit.set(0)
            self.skip_strings.set(True); self.strings_lite.set(False); self.recursive_rounds.set(0)
        self._apply_effective_line_only()
        self._update_strings_warning()
    ...
    def browse_sample(self) -> None:
        p = filedialog.askopenfilename(title="Choose an EXE/DLL", filetypes=[("Windows executables", "*.exe *.dll"), ("All files", "*.*")])
        if p:
    ...
    def run_analysis(self) -> None:
        ...
        threading.Thread(target=self._run_worker, args=(sample, case), daemon=True).start()
    ...
    def _run_worker(self, sample: str, case: str | None) -> None:
        try:
            ...
            def apply_success_updates() -> None:
                if cd:
                    self.current_case_dir = Path(cd)
                    summary_path = Path(cd) / "summary.json"
                    try:
                        if summary_path.exists():
                            ...
                            if isinstance(sample, dict):
                                if sample.get("md5"):
                                   self.set_step_state("md5", "done", 100.0)
                                   self._step_labels["md5"].configure(text=f"MD5: {sample['md5']}")
                                if sample.get("sha1"):
                                   self.set_step_state("sha1", "done", 100.0)
                                   self._step_labels["sha1"].configure(text=f"SHA1: {sample['sha1']}")
                                if sample.get("sha256"):
                                    self.set_step_state("sha256", "done", 100.0)
                                    self._step_labels["sha256"].configure(text=f"SHA256: {sample['sha256']}")
                    except Exception as exc:
                        self.log(f"[!] WARN: could not refresh hash rows: {exc}")

                self.set_step_state("case", "done", 100.0)
                self.status_text.set(f"Done. Verdict={verdict} Score={score} PDF={'yes' if pdf else 'no'}")
                self.log(f"Completed: verdict={verdict} score={score} case_dir={cd}")

            self.after(0, apply_success_updates)
        except Exception as e:
            err_msg = str(e)
            self.after(0, lambda: self.status_text.set("Failed."))
            self.after(0, lambda msg=err_msg: self.log(f"[!] ERROR: {msg}"))
            self.after(0, lambda msg=err_msg: messagebox.showerror("Analysis failed", msg))
        finally:
            self.running = False
