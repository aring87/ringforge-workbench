...
the C2 is loopback, so FakeNet structurally cannot reach it -- accepted one
connection and the whole stack fell out:
...
**Two documented claims were wrong, and both were ours.**

*"Server speaks first"* is retracted. The client opens with a TLS ClientHello --
`16 03 01 01 9d`, `supported_versions` offering TLS 1.3 down to 1.0, x25519 and
secp256r1, no SNI. The claim came from reading the sample; nothing had ever
accepted its connection, so nobody had watched it.

*`0xDEADBEEF` on the wire* is retracted as a description of the transport. It is
the **inner** protocol, invisible under TLS -- which is why the silent run
recorded five ClientHellos that the frame parser flagged as "Invalid magic".
**That parser reporting instead of raising is what preserved them**; the sample's
own `Decode` throws there, and a parser that copied it would have discarded the
first observation of this protocol for disagreeing with a model built from IL.

**It does not validate the certificate.** The handshake completed against a
self-signed leaf minted minutes earlier for `win11` and `127.0.0.1`, from a CA
the guest has never trusted. No CA was installed; the trust store is unmodified.
**Anything that can bind 7372 is its C2.**

**The frame model is confirmed**, having been IL-derived the day before:
`21 01 00 00` payload 289, `84 01 00 00` original 388, flag 1, Deflate, and
13 + 289 + 4 = 306. The CRC polynomial was an explicit assumption and **standard
IEEE CRC-32 matched first try**.

**The message is `Packet: listinfo`** -- an operator-panel check-in. 14 declared
pairs, 14 decoded, 388 of 388 bytes consumed:

    IP 192.168.56.20   Group Started        Country Unknown
    UID Raton_Fcm7JziU Username adam        Machine WIN11
    Os Windows 11 Pro 64bit                 Executing Administrator
    AV Windows Defender                     Pass (empty)
    Version Free       Clock 21:15:51 01/09/2026
    Payload ce0d08be...exe

**`Version: Free`** says the family sells tiers and this is the unpaid one.
**`UID: Raton_Fcm7JziU`** is a prefix plus eight random characters, generated by
exactly the routine `stuff.dll` carries (`lengthRandom`, `charsRandom`). `Raton`
is Spanish for mouse, and `stuff.dll`'s PDB path names the developer account
`Cristian`: two independent signals of Spanish-language authorship, and a strong
lead for attribution, **which has not been attempted**.

The fields match the config schema recovered from `stuff.dll` a day earlier --
`UID`, `Group`/`Tag`, `Pass` -- so the builder's options and the check-in are
the same set seen from both ends.

**And the beacon stops once it lands.**

    refused                    every 17.03 s   (212 in an hour, stdev 0.014)
    accepted but unanswered    every 35 s
    accepted and delivered     once, then silence for 3.5 minutes

The loop is "retry until the check-in is delivered", not a heartbeat. What it
does after a *reply* is unknown -- nothing has ever answered it, and the panel
would normally send commands. `beacon_frame.py` can now build a frame and
`decode_dictionary` can read one, so a responder is a small step.
