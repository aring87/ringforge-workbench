...
Stage 4's *behaviour*. It runs to completion and returns without dropping a
file, opening a socket or touching the registry in this environment, which is
consistent with one exchange of a protocol that may have several. `--survey`
showed it zeroing two 64 KB regions on the way out, so it does clean up after
itself.
