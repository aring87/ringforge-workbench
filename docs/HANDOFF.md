...
### Pick up here — 02 Sep, end of session

**`ce0d08be...` is done as an analysis subject, and that is a decision rather
than an exhaustion.** What it had to give has been taken: persistence and its
self-healing, the full capability set, the C2 protocol end to end, the check-in
contents, a driven command channel, the VM reporting, a confirmed ransom
capability, a DoS bug, and IOCs. What remains is optional and named at the
bottom of this section.

**The day, in the order it happened.**

    the gate            AutoAdminLogon=0 removes the race instead of racing it.
                        ONSTART fired at 146, 181, 231 and 313s across four
                        boots -- 167s of spread, which is why no trigger works
    boot window         Sysmon since-boot, no race at all, with the payload's
                        own offset measured on every run
    the persisted stage watched for the first time: 205s margin, self-healing
                        schtasks persistence, Costura unpacking AForge and
                        NAudio -- webcam and microphone
    the beacon          127.0.0.1:7372, 212 attempts in an hour, 17.034s mean,
                        stdev 0.014, no jitter at all
    stuff.dll           the protocol core: 0xDEADBEEF, lengths, a compression
                        flag, CRC-32, and a config schema
    the C2              client speaks first, in TLS, and does not validate the
                        certificate. Anything that can bind 7372 is its C2
    the command channel seven commands answered, then thirteen recon ones.
                        Clientinfo reports the VM to the operator
...
**Snapshots, and which to use.**
    ...
    ce0d08be-armed-gated-tooled   USE THIS. Armed, gated, with the beacon
                                  tooling and certs baked in
    ce0d08be-armed-gated          the same without the tooling; cited as
                                  provenance by three artifact READMEs, so it
                                  is kept rather than renamed
    ce0d08be-gated-run-complete   post-run state, redundant now
    ce0d08be-armed-gated-bootlog  the failed first gated attempt, kept as the
                                  evidence for the boot-log trap
    tooling-baseline              clean, at clone 9ed26b8, with pytest and
                                  unicorn in it

**A trap that costs a restore if forgotten.**
`ce0d08be-installed-onlogon-armed` carries `PROCMON24 Start=0`, so **every
restore of it boot-logs once** -- ~4 GB into `C:\Windows\Procmon.pmb`, which
then blocks any Procmon start with a modal. Its descendants do not.

**Guest state.** `ce0d08be-armed-gated-tooled` restored and powered off,
contained, at the sign-in screen when booted. No payload running. Whether
`C:\beacon-*` output directories were cleared before that snapshot was taken is
not verified.
...
the payload had not started beaconing), `Notify` blamed for `Report`'s crash,
`Jigsaw` read as the ransomware when `Encrypt` is, and responses paired by
position when every one names the command it answers. Each was "the thing that
happened next belongs to the thing that happened last", and each was caught by
...
**Report shells, all seven.** Four hand-rolled their own `<!DOCTYPE html>`
around `report_css()` instead of calling `report_page`, so they had no banner,
verdict, footer or timestamp. `report_page` gained an **optional verdict** --
two of them have none, and the fix is to let a report say nothing rather than
invent a band -- and a footer note naming the producing module. Consolidated:
`extension_window` (a byte-for-byte re-implementation), `verdict_report`,
`dynamic_window`'s fallback, and `api_window`. Only `report_theme` contains a
page shell now.
...
**Manual API Tester -- `static_triage_engine/api_report.py`.** Its report lived
inline in the window. `analyze_response` returns structured findings with
severities and the report was embedding the *plain-text rendering* of them in a
`<pre>`, so the one module whose purpose is scoring a response was the only one
that could not show a severity. Now: findings as badge-marked rows worst-first,
headers as tables with malformed lines kept and marked, status families
coloured, and a loud redaction notice that counts what was actually replaced.
21 tests.

**Unified Report -- `verdict/case_summary.py`.** Four widget-free functions left
the window and took two defects with them:

* **13 of 13 verdicts rendered grey.** "High Risk", "High API Spec Risk" and
  "High Browser Extension Risk" all mapped to `sev-none`, the same chip as "No
  Results". `severity_class_for_label` knows those vocabularies now, retired
  wording included so old case folders still colour.
* **A band derived by grepping prose.** The fallback joined every finding's text
  and matched substrings, so `persistence` anywhere in any module's output
  produced *"Moderate Risk"*. Deleted; it reports canonical coverage wording
  instead, distinguishing "nothing collected" from "ran and found nothing".

The per-domain arithmetic moved **unchanged** -- whether a spec case should band
on the corroboration model is a product decision, and altering numbers under the
same names would move verdicts silently. A test now pins the surprise inside it:
...
### Pick up here — 04 Sep, the workbench became something you can ship

**A different subject again: this is packaging, distribution and integration,
not analysis.** The module-polish pass above finished, and then the day was
spent turning a working bench into a product somebody else could install.

**It is installable.** `pyproject.toml` declares the five things the engine
actually imports rather than the `pip freeze` in `requirements.txt`, which pins
PyInstaller and the scientific stack that only `scripts/` touches. Both files
stay: one describes the package, the other reproduces a machine. Collectors
with a documented degradation path are extras, so "not installed" keeps being
a decision on record.

**Packaging found a layering defect the suite structurally could not.** Five
...
`sys.modules`, so all ~16 importers are untouched and private names still
resolve.
...
environment somewhere else and runs `scripts/check_installed_package.py`, which
exists because no test run from the repo root can ever catch the class of bug
above.

**There is a command line.** `ringforge` is the CLI, `ringforge-gui` the
window.
...
**And the CLI immediately found a defect in `verdict/model.py`.** Pointed at an
empty directory it reported `No Indicators Found / Low` -- the cleanest band the
model can produce, for a folder containing nothing. The `NOTHING_COLLECTED`
guard read `if not present and categories and not collected_any`, so it covered
a run whose collectors all *failed* and missed the run that never *happened*.
The comment directly above it already called this "the project's most expensive
recurring mistake wearing a new hat". It was, one line lower. The existing test
was named `test_nothing_at_all_is_insufficient_coverage` and asserted `Low`.

**A verdict now says what it describes and what produced it.** At
`schema_version` 1.0: `generated_utc`, `case_id` (the sample sha256, so a
consumer updates rather than duplicates), `case_name`, a `sample` block, and
`provenance` carrying the analyzer version, commit, library versions and what
the YARA scan actually compiled. `case_dir` is gone -- an absolute Windows path
that identified the analyst's machine and was read by nothing.

The `provenance.collectors.yara` block earns its place. Against the archived
20 Aug artifact and today's:
...
**`ringforge export` maps a verdict to an OCSF Detection Finding** (class_uid
2004) and appends NDJSON for a forwarder to ship. Nothing opens a socket. The
mapping exists to carry **coverage**: `Unknown` maps to OCSF severity 0
...
incomplete*. That is the distinction this whole model exists to draw, surviving
one system boundary further out.

**Licensing is recorded rather than assumed.** `THIRD-PARTY-NOTICES.md` reads
versions and licences from installed distribution metadata, marks what it could
not verify as `verify`, and flags two rows for a lawyer instead of answering
them. PySimpleGUI is out of `requirements.txt`: version 5 is Proprietary and
nothing imported it. The redistribution posture was already right and is now
written down -- Procmon, Autorunsc and Sysmon may not be redistributed and
`.gitignore` excludes their binaries, verified against `git check-ignore`. The
likeliest snag is the neo23x0 signature-base YARA set, non-commercial for a
substantial part of it and redistributed by nothing here.

**The guest bench is tooled and baselined.** See *The snapshot tree*.
`tooling-baseline-0b74864` carries the full tier-1 set, verified after a reboot,
contained. `capture_status()` now probes the Npcap driver rather than inferring
it from `dumpcap.exe` -- the same shape as `rule_file_count`, found because the
driver had to be checked by hand during the bootstrap.
...
* **A labelled corpus and a confusion matrix.** Per-category benign rates exist
  and are cited throughout `categories.py`; the *bands* have never been measured
...
* The spec domain's two views disagree -- see *Worth a look, spec*. A product
...
**The orchestrator now reports the gap it cannot close.** A run that installs
persistence with a future logon or boot trigger has a stage it will never watch
-- `run_dynamic_analysis` lives inside one boot in the guest, while the gated
capture spans a reboot and needs the host to drive a logon. So it detects the
condition, writes `deferred_stage` into the run summary, emits a status line,
and carries the exact procedure for the gated run.

It does **not** run the capture, and that boundary is deliberate: a function
claiming to observe the deferred stage while structurally unable to would be
worse than one that declines and hands over the steps. `dynamic_analysis/
deferred_stage.py`, 14 tests, suite at 1,030.

The measured case it exists for: `ce0d08be...` scored Likely Malicious at 140
on its installer stage while the resident RAT that its `ONLOGON` task launches
-- beaconing every 17.03 s, reporting the machine to an operator, carrying a
ransom capability -- was in no run at all, and nothing in the report said so.

**What this closes.** The module question from earlier today. The gate, the
boot-window collector, the listener and the responder remain operator-driven
tooling by design; what is now wired into the pipeline is the *signal that they
are needed*, which is the part a run can honestly provide.

### Pick up here — 02 Sep, the command channel is driven

**Seven commands were sent down one TLS session and answered.** The first time
anything has instructed this sample. `Ping`, `Clientinfo`, `Request`, `Report`,
`Survival` acted; `Connect` and `Task` returned "Unknown packet type".

**`Clientinfo` reports VM identifiers to the operator** -- `innotek GmbH`,
`Oracle Corporation VirtualBox`, the VirtualBox WDDM adapter, a MAC beginning
`080027`. These are **WMI queries inside a command handler**, so they run only
when a C2 asks.

**That explains this sample's gap 4 silence without reopening gap 4.** 967
registry reads with no VM artefact remains true, and so does the reason gap 4
was declined -- the registry surface is rare and OS-swamped. What was an open
thread is now answered: with no C2 the VM collection never runs, and every
prior run had no C2. It is **reporting, not an evasion gate**; nothing observed
shows behaviour changing on it.

**The dispatcher is a discovery oracle.** "Unknown packet type: <name>" means
...
attribution and was wrong. Decompiled, `case "Report"` reads `Name`, gets null
from a bare packet, and passes it to `ProcessMonitor.Start`; the exception
surfaces on a background thread a second later, so the command *in flight*
looked like the cause. A free denial of service against this family, and the
...
**Where a sweep resumes.** 132 safe candidates, 8 tried. To continue, log the
guest off and on (or reboot from `ce0d08be-armed-gated`) to get a fresh payload,
then run with a `commands.txt` starting after `Notify`. Withholding stays on by
default; `--include-destructive` sends the 33 held names, which include `Jigsaw`
beside `Encrypt`/`Decrypt`.

**Caused by this run, not by the sample:** a report window and the folders
`Survival` created.

Read, in `docs/ROADMAP.md`:

    Item 1 - the command channel, driven, and why gap 4 could never have seen
             the VM check

Artifact: `G:\ringforge-artifacts\ce0d08be-command-channel-02sep\`.

### Pick up here — 02 Sep, the family is Raton and the responder is armed

**The payload names itself.** `.BotKillerRaton`, the `Raton_` UID prefix, and
"your raton client message box" -- matching `Raton_Fcm7JziU` on the wire and the
`Cristian` PDB path in `stuff.dll`. **Attribution against public reporting has
not been attempted.**

**Its strings were read without writing the file to disk** -- Bitdefender
quarantined the extracted copy in seconds, and the policy here is a password zip
rather than an AV exclusion, so the bytes went from 7-Zip's stdout into
`DotNetImage(data=...)`. 1,626 user strings, 3,697 identifiers, on
`G:\ringforge-artifacts\ce0d08be-payload\`.
...
sent as empty**. A password the client holds and does not send is a plausible
auth step on the reply path, and that is exactly what the responder tests next.

**Defence evasion nobody had seen it do:** it runs
`powershell -ExecutionPolicy Bypass "Add-MpPreference -ExclusionProcess ..."`
to whitelist itself in Defender, and carries a second persistence path under
`...CurrentVersion\Run` beside the `schtasks` one that was observed.

**THE RUN TO DO FIRST.** The guest is up with the payload resident and the
listener staged. In the guest:

    Copy-Item \\VBOXSVR\ringforge\gated-run\tls-commands.txt C:\tls\commands.txt -Force
    python scripts\beacon_listener.py --out C:\beacon-reply --minutes 10 ^
        --tls-cert C:\tls\server.pem --tls-key C:\tls\privkey.pem ^
        --respond --commands C:\tls\commands.txt

Watch for `ANSWERED:` -- bytes back after our reply mean the client parsed it,
and those bytes are the command channel, which nothing has ever seen.

**Keep the distinction.** The capability list is *strings in an image*. What
this sample has been *observed* doing is the `schtasks` persistence and its
re-creation, 967 registry reads with no VM artefact, a 17.03 s beacon, one TLS
check-in, and the Costura unpack. Nothing has shown it being told to do the rest.

Read, in `docs/ROADMAP.md`:

    Item 1 - the family names itself, and 165 commands come out of its strings

**Housekeeping:** Bitdefender quarantined a loose copy of the payload from
`G:\ringforge-artifacts\ce0d08be-payload\extracted\`; the empty directory may
still be there and the AV's quarantine holds the file. `payload.zip` beside it
(password `infected`) is the copy to keep.

### Pick up here — 02 Sep, the C2 protocol is understood end to end

**306 bytes answered it.** A listener bound to 7372 *inside the guest* -- the C2
is loopback, so FakeNet structurally cannot reach it -- took one connection and
the whole stack fell out:

    TCP    127.0.0.1:7372
    TLS    client-initiated, TLSv1.3, TLS_AES_256_GCM_SHA384
    frame  0xDEADBEEF, lengths, compression flag, payload, CRC-32
    body   [count] then count x [len][key][len][value], UTF-8

**It does not validate the certificate** -- the handshake completed against a
self-signed leaf from a CA the guest has never trusted, and no CA was installed.
**Anything that can bind 7372 is its C2.**

**The message is `Packet: listinfo`**, an operator-panel check-in: `IP`, `Group`,
`Country`, `UID Raton_Fcm7JziU`, `Username adam`, `Machine WIN11`,
`Os Windows 11 Pro 64bit`, `Executing Administrator`, `AV Windows Defender`,
`Pass` empty, **`Version Free`**, `Clock`, `Payload`. Its fields are the config
schema recovered from `stuff.dll` the day before, seen from the other end.

`Raton` plus the `Cristian` PDB path are two independent signals of
Spanish-language authorship and a strong attribution lead. **Not attempted.**

**Two of our own claims were retracted by this run.** "Server speaks first" was
read from the sample rather than watched, and is wrong -- the client opens with
a ClientHello. And `0xDEADBEEF` is the *inner* protocol, invisible under TLS;
the silent run's five "Invalid magic" frames were ClientHellos, preserved only
because `parse_frame` reports where the sample's own `Decode` throws.

**The beacon stops once the check-in lands** -- 17.03 s when refused, 35 s when
accepted and unanswered, then silence after one successful delivery. It is a
retry loop, not a heartbeat.

**The next step is the reply.** Nothing has ever answered `listinfo`, so the
command half of the protocol is unobserved. `dynamic_analysis/beacon_frame.py`
builds frames and `decode_dictionary` reads bodies, so a responder is small --
and `Pass` being empty here leaves untested whether a password is enforced.

Read, in `docs/ROADMAP.md`:

    Item 1 - the C2 protocol, end to end, and the first message it ever sent

Artifacts: `G:\ringforge-artifacts\ce0d08be-c2-checkin-01sep\` (this),
`...-beacon-capture-01sep\`, `...-gated-capture-01sep\`, `...-costura-01sep\`.

### Pick up here — 01 Sep, the beacon is measured

**The beacon run finished and answered its question.** 212 connection attempts
in an hour, **all to `win11:7372`** -- loopback, port 7372, never varying. The
hardcoded C2 from static analysis is confirmed dynamically for the first time.

    interval   mean 17.034s, median 17.033s, stdev 0.014s, zero outliers
    shape      4 x TCP Reconnect + 1 x TCP Disconnect per attempt
    cost       8.8 MB backing, 2.0 MB CSV, for a full hour

**Two things this corrects, both mine, both from reading the socket table as
more precise than it is.** The "17 or 18 seconds, true period ~17.5" jitter was
`CreationTime` resolving only to the second -- the timer is 17.03 s flat, which
makes it *easier* to detect, not harder. And "one leaked socket per attempt,
monotonic, 77 hours to port exhaustion" is withdrawn: four sockets existed at
20:51 against 212 attempts in the hour before, so they are released.

**There is no `TCP Connect` event in the whole capture** -- Procmon logs that
only for a connection that completes, and a SYN into a closed port yields
`TCP Reconnect` and `TCP Disconnect`. A rule watching `TCP Connect` would have
seen nothing while the sample beaconed 212 times.

**The next thing is the content, and it needs a listener.** Nothing answers on
7372, so every attempt dies at the SYN and no application data is ever sent.
`stuff.dll` gives the frame it would send -- `0xDEADBEEF`, compressed length,
original size, compression flag, payload, CRC-32 -- and confirming that on the
wire is a `beacon_responder.py` or FakeNet job. It is also the first chance to
answer a server-speaks-first protocol and see what the client sends once it
believes it has a C2.

Read, in `docs/ROADMAP.md`:

    Item 1 - the beacon goes to 127.0.0.1:7372, every 17.03 seconds, without jitter

**Unexplained and worth keeping:** the payload was beaconing minutes after
logon on this boot, against 88 minutes measured on an earlier one. Either the
earlier gap was a phase in which sockets were closed rather than retained, or
the schedule depends on disk state -- this boot came from a snapshot on which
the payload had already run twice.

**Guest state.** `ce0d08be-armed-gated` restored and running, contained, logged
in as `adam`, payload resident as PID 5424 since 18:43:25, capture finished.
Nothing needs it in that state. Snipping Tool was opened in the guest during the
run and its process events are in the capture as analyzer-side noise.

### SUPERSEDED — the beacon run, as it was set up

**A capture is running or about to be, and this is the first thing to check.**
The question it answers: **where do the beacons go**. `ce0d08be...` attempts a
connection every 17 seconds and leaks a bound socket per attempt, and no
instrument has yet shown the destination -- a bound socket carries no remote
endpoint, Sysmon's config excludes loopback, and the gated Procmon capture
watched six and a half minutes of a behaviour that starts around 88 minutes in.

**Setup, as left.** `ce0d08be-armed-gated` restored, contained (NIC1 cable off),
booted to a sign-in screen with `AutoAdminLogon=0`. The operator logs in as
**`adam`** by hand -- that moment is `t=0` for the payload -- waits ~60 minutes,
then runs, elevated, in the guest:

    python scripts\logon_capture.py --capture --out C:\netcap ^
        --procmon ...\tools\rf_trace64.exe --window 3600 ^
        --config ...\tools\procmon-configs\dynamic_network_only.pmc

A 3,600 s window from t+60 brackets the 88-minute mark. `dynamic_network_only.pmc`
is new (`bae7697`): twelve operations -- the TCP set, UDP send/receive, and the
three process operations that keep lineage -- and nothing else, so it is cheap
enough to leave running for hours where the registry-read config is not
(~29 MB/min, 1.7 GB an hour).

**What a result looks like:** `TCP Connect` rows at 17-second intervals with a
destination. **What a null looks like, and what it would not mean:** no TCP rows
in an hour would say the 88-minute figure is wrong or the loop had stopped, not
that the payload is quiet -- the socket table is the check for that
(`Get-NetTCPConnection -OwningProcess <pid>`), and its `CreationTime` values are
the beacon schedule.

**Two things that are set up and will look like faults if they are not
expected.** The armed `ONSTART` task fires ~180 s into that boot and runs its own
600 s registry capture against a machine nobody has logged into -- harmless,
finished long before the network capture starts, ~290 MB into `C:\logon-capture`.
And the network capture's manifest says `captures_registry_reads: false`, which
is correct and deliberate: a run under that filter showing no VM artefact has
not looked for one.

### Pick up here — 01 Sep, evening

**The gated capture ran, and the persisted stage was watched by a collector that
was already recording when it started.** Boot 17:31:02, capture confirmed and
signalling at 17:34:03 (181 s), `ONLOGON` payload at 17:37:28 as PID 5432 --
**a 205-second margin, measured**, because the logon could not happen until it
was allowed to. 1,341 payload events against the 31 Aug boot log's 1,438 for the
same stage, from two unrelated instruments.

**The RAT does webcam and microphone surveillance.** It unpacks a Costura.Fody
dependency set to `%TEMP%\Costura\43C0B7C2D694D39E487F61CFD3964734\`:
`aforge.video.directshow.dll`, eight `naudio` assemblies, `newtonsoft.json.dll`,
and a 12 KB `stuff.dll` that is its own and has not been looked at. None of it
appears in the executable's imports -- Costura carries them as compressed
resources -- so no static pass could have found it and no earlier run watched
the stage that unpacks it. Archived with SHA-256s at
...
Read, in `docs/ROADMAP.md`:
    ...
    Item 1 - the gated capture ran, and the RAT has a webcam and a microphone
    Item 1 - the manifest asserted a capability the capture did not have

**Snapshots.** `ce0d08be-armed-gated` is the state the run starts from and is
repeatable. `ce0d08be-gated-run-complete` is what it left behind.
`ce0d08be-armed-gated-bootlog` is the failed first attempt, kept because it is
the evidence for the trap below.

**A trap worth knowing before restoring anything in that lineage.**
`ce0d08be-installed-onlogon-armed` carries `PROCMON24 Start=0`, so **every
restore of it boot-logs once** -- ~4 GB into `C:\Windows\Procmon.pmb`, which
then blocks any Procmon start with a modal. That is what defeated the first
gated attempt. Both refusals behaved correctly: the capture reported
`blocked by a Procmon boot log`, and the host declined to type credentials into
a machine that was not recording.

### Pick up here — 01 Sep

**Two collectors were built for the persisted stage and neither has run against
a guest.** 955 tests, no detonation. Everything below is code and reasoning; the
measurements it exists to produce have not been taken, and the first one to take
is `-ProveChannel` on the clean baseline.

**The trigger problem is answered by not having a race.** The 31 Aug entry below
leaves the collector needing "a boot-start service or a filtered boot log". Both
are attempts to *win* the race against the sample's `ONLOGON` task, and both
leave a margin to re-prove on every sample. So `AutoAdminLogon` goes to 0 before
the boot: the guest stops at the sign-in screen, no logon session exists, the
sample's task does not fire, and the capture can be as late as Task Scheduler
likes. It confirms its own backing file is growing, sets a guest property, and
the host types the credentials from there. `ONSTART` is still the trigger and is
no longer load-bearing.

    guest   scripts\logon_capture.py --arm --gate-logon   (also --gate/--ungate)
    host    scripts\vm_gated_logon.ps1 -Snapshot <snap> [-ProveChannel]

**Sysmon never had the race, and that is the cheaper half.** Its driver is
boot-start, so it was recording before the logon on every run this project has
done -- both `ce0d08be...` detonations included. `scripts\sysmon_since_boot.py`
reads the whole boot window and attributes it by image name. **The persisted
stage of those two runs may already be in the guest's event channel**, which
would answer "what does the RAT actually do" with no new detonation at all.
That is the cheapest open thing on this list. It covers process creates, image
loads, network, DNS, registry *writes*, pipes and clipboard -- and no reads at
all, which every manifest states as `reads_covered: false`.

**One number decides whether a gated run counts**, and it is checkable rather
than assumed, which is the whole point of the change:
...
**Gap 4 is closed, by measurement, and exit criterion 4 is satisfied.** Its
active detector is *declined with a reason* rather than built-and-unproven. Two
detonations of `ce0d08be...` -- the only 1 of 226 malware samples that names a
VM-only registry key in its own bytes -- watched the installer stage and then,
across an `ONLOGON` scheduled task, the payload stage. The payload ran resident
and elevated for an hour and made **967 registry reads, none of them a VM
artifact**, on a capture that demonstrably saw it (1,438 events, 62 image loads,
its own process start). Against that, Windows itself makes **~450 `vm_specific`
reads per ordinary boot**. The surface is rare in the field and swamped by the
OS where it appears.

Read, in `docs/ROADMAP.md`, in this order:

    Item 1 - the sample for gap 4, chosen by measurement
    Item 1 - the run, and what it actually measured
    Item 1 - the logon run, and gap 4 declined with a reason
    Item 1 - the collector's proving run failed, and what it falsified

**What the investigation exposed is bigger than gap 4 and is the open work.**
The pipeline detects persistence but observes only one boot, so *what a
persisted payload does* is outside every dynamic detector. `ce0d08be...` scored
Likely Malicious at 140 on its installer stage alone; the RAT itself -- resident,
beaconing, stealing -- lives in the copy its `ONLOGON` task launches and had
never been watched.

`dynamic_analysis/logon_capture.py` and `logon_analysis.py` are the collector
built against that. **They are demonstrated not to work yet, for two measured
reasons**, and that is where tomorrow starts:

1. **The trigger is wrong and needs a different mechanism, not a better flag.**
   `ONSTART` was chosen on the reasoning that it runs before any user session
   exists. Measured: the payload started 21:32:55, the capture 21:36:46. Task
   Scheduler delays boot-triggered tasks, and the payload is up 2 seconds after
   the logon. Candidates are a boot-start service, or Procmon boot logging if it
   can be made to honour a filter -- unfiltered boot logging is measured at
   65 MB/min, which is why it was rejected as a collector.
2. **Procmon's modals block a session-0 start** and `/Quiet` suppresses none of
   them. Handled now: `check_blocking_prompts` clears our own backing file and
   refuses on a boot log with the remedy, and the backing file is verified after
   launch. Under the same command interactively Procmon captures fine, so the
   command was never the fault.

**One thread on `ce0d08be...` is named rather than open-ended.** Its VM check may
sit behind a completed C2 handshake -- `"Client ... was a virtual machine!"` is
operator-facing panel text. That cannot be tested with this sample: its C2 is
hardcoded `127.0.0.1:7372` and the protocol is server-speaks-first. A configured
sample of the family would be needed.

**Guest state.** `tooling-baseline` is clean, contained, powered off, and now
carries the cleared Procmon config plus the repo at `3e0dd52` and
`tools\rf_trace64.exe`. Tonight's fixes (`851d026`, `4e7a219`) are *not* in it --
pull and re-baseline. `ce0d08be-installed-onlogon-armed` appears to be the only
snapshot still holding the malware corpora; do not delete it without checking,
and consider taking a clean corpus baseline from it.
...
| `procmon_filter` in the summary | **Proven, twice, 31 Aug.** Reported `captures_registry_reads: true` with all 18 operations named on both runs, and it is the field that made an empty VM-artifact result readable as a statement about the sample rather than about the collection. Earlier: **built, unproven.** Which filter ran, its operations, and whether reads were captured — the setting a whole pass turns on and the only capture setting that was not in the record. Read from the file, not the filename |
...
| Gap 4's active detector (`correlate_vm_check_with_silence`) | **Declined with a reason, 31 Aug — exit criterion 4 satisfied.** Exists, 14 tests, wired into `orchestrator.run`, renders as a `card-alert`, and its collection path is now proven twice (171,728 reads, then 3.39 million). Never fired, and the input does not occur: 1 of 226 malware samples names a VM-only registry key, 0 of 3 detonated for this read one, and Windows makes ~450 `vm_specific` reads per ordinary boot. `QUIET_EVENT_THRESHOLD` stays uncalibrated and now always will |
| `vm_specific` without lineage | **Measured and unusable, 31 Aug.** 448 VM-artifact reads on one ordinary boot, every one background — `services.exe` enumerating every VBox, vmbus and vmic service key, Defender and `WMIADAP` probing `\Performance` under each, Explorer and winlogon and three PowerShells on `VBoxSF\NetworkProvider`. Lineage does all the work, which is what a logon-triggered payload takes away. `ROUTINE_SUBPATH_MARKERS` is measurably too narrow and deliberately not widened: it can only hide a hit, and no run has a known positive to measure the cost against |
| Logon capture (`logon_capture.py`) | **Built, and demonstrated not to work — 31 Aug.** The task fired and the manifest was written; there was no `.pml`. Two measured causes: Procmon's modals block a session-0 start, and `ONSTART` ran 3m51s after the `ONLOGON` payload it was meant to precede. Prompts and self-verification are fixed with tests; **the trigger is not** and needs a boot-start service or a filtered boot log |
| Logon analysis (`logon_analysis.py`) | **Unit-proven, live-unexercised.** Drives the same five passes `orchestrator.run` drives, so a logon capture and a detonation cannot drift. Lineage needed no new code: `_mark_sample_lineage` already seeds on the sample's image name, so a payload with no launched PID resolves from `sample_pid=None` plus a name |
